...
export function DashboardPage() {
  const { groupedAnalyses, filteredAnalyses, filters, setFilters, isLoading, error, isFallback } = useAnalyses();
  const averageImpact =
    filteredAnalyses.length === 0
      ? 0
      : Math.round(filteredAnalyses.reduce((sum, item) => sum + item.impactScore, 0) / filteredAnalyses.length);
  const averageMove =
    filteredAnalyses.length === 0
      ? 0
      : Math.round(filteredAnalyses.reduce((sum, item) => sum + item.expectedMove, 0) / filteredAnalyses.length);
  ...
  return (
    <div className="dashboard-page">
      <PageHeader
        ...
        meta={isLoading ? 'Synchronizing' : `${filteredAnalyses.length} analyses`}
      />
      ...
      <section className="metrics-grid" aria-label="Dashboard metrics">
        <MetricTile label="Analyses" value={String(filteredAnalyses.length)} accent="cyan" />
        <MetricTile label="Avg Impact" value={String(averageImpact)} accent="green" />
        <MetricTile label="Avg Move" value={averageMove > 0 ? `+${averageMove}` : String(averageMove)} accent="pink" />
